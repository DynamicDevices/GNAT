...
#undef NDEBUG // Enable Assertions for ESP_ERROR_CHECK()
...
void openhaystack_main(void)
{
    ...
    if ((status = esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT)) != ESP_OK) {
        ESP_LOGW(LOG_TAG, "esp_bt_controller_mem_release: %s", esp_err_to_name(status));
    }

    if ((status = esp_bt_controller_init(&bt_cfg)) != ESP_OK) {
        ESP_LOGE(LOG_TAG, "esp_bt_controller_init: %s", esp_err_to_name(status));
        return;
    }
    ...
    if ((status = esp_bluedroid_init()) != ESP_OK) {
        ESP_LOGE(LOG_TAG, "esp_bluedroid_init: %s", esp_err_to_name(status));
        return;
    }

    if ((status = esp_bluedroid_enable()) != ESP_OK) {
        ESP_LOGE(LOG_TAG, "esp_bluedroid_enable: %s", esp_err_to_name(status));
        return;
    }
    ...
    if ((status = esp_ble_gap_register_callback(esp_gap_cb)) != ESP_OK) {
        ESP_LOGE(LOG_TAG, "esp_ble_gap_register_callback: %s", esp_err_to_name(status));
        return;
    }
